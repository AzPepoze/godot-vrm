...
func create_runtime(secondary_node: Node3D, skeleton: Skeleton3D) -> VrmRuntimeCollider:
	...
	var bone_idx: int = -1
	if node_path != NodePath() and secondary_node != null:
		node = secondary_node.get_node_or_null(node_path)
...
class CapsuleCollider:
	...
	func update(
		...
		if bone_idx != -1:
			...
			if node:
				position = (
					p_center_transform * p_skel_global_xform_inv * node.global_transform * offset
				)
				tail_position = (
					p_center_transform
					* p_skel_global_xform_inv
					* node.global_transform
					* tail_offset
				)
			else:
				position = Vector3.ZERO
				tail_position = Vector3.ZERO
	...
	func draw_debug(mesh: ImmediateMesh, center_transform_inv: Transform3D) -> void:
		...
		var up_axis: Vector3 = (tail_position - position).normalized()
