...
void draw_gizmo(ImmediateMesh *mesh, Skeleton3D *skel,
  ...
  if (!impacts.empty()) {
    ...
    for (const auto &impact : impacts) {
      float alpha =
          1.0f - (impact.age / SpringBoneConstants::IMPACT_FADE_DURATION);
      if (alpha <= 0)
        continue;
      ...
      Vector3 norm_gizmo =
          skel_to_gizmo.basis.xform(skel_global_inv.basis.xform(impact.normal));

      // Draw a cross at impact point
      float size = 0.05f;
      draw_line(mesh, pos_gizmo + Vector3(size, 0, 0),
                pos_gizmo - Vector3(size, 0, 0), impact_color);
      draw_line(mesh, pos_gizmo + Vector3(0, size, 0),
                pos_gizmo - Vector3(0, size, 0), impact_color);
      draw_line(mesh, pos_gizmo + Vector3(0, 0, size),
                pos_gizmo - Vector3(0, 0, size), impact_color);

      // Draw normal vector
      draw_line(mesh, pos_gizmo, pos_gizmo + norm_gizmo * 0.2f,
                Color(1, 1, 0, alpha)); // Yellow normal
    }
    mesh->surface_end();
  }
}
