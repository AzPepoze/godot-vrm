...
void VRMSpringBoneSimulation::_query_game_object_collisions(
  ...
  if (Math::abs(axis.dot(Vector3(0, 1, 0))) > 0.9999f) {
    rot_basis = (axis.dot(Vector3(0, 1, 0)) > 0.0f)
                    ? Basis()
                    : Basis(Vector3(1, 0, 0), Math_PI);
  } else {
  ...
  TypedArray<Vector3> contacts =
      space_state->collide_shape(env_query_params, 32);
  if (contacts.size() < 2)
    return;
  ...
  for (int i = 0; i < contacts.size() - 1; i += 2) {
    ...
    if (depth > max_depth) {
      ...
      if (bone_length_sq > 1e-8f) {
        ...
        float t =
            (contact_on_shape - origin_world).dot(bone_dir) / bone_length_sq;
