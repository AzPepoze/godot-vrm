...
static void draw_line(ImmediateMesh *mesh, const Vector3 &begin, const Vector3 &end, Color color) {
...
static void draw_wireframe_sphere(ImmediateMesh *mesh, const Vector3 &center, float radius, Color color) {
    if (radius <= 0.0f) {
        return;
    }
    const int step = 15;
    ...
    if (draw_spring_bones && !chains.empty()) {
        ...
        for (const auto &chain : chains) {
            ...
            for (const auto &joint : chain.joints) {
                ...
                Vector3 end_gizmo = skel_to_gizmo.xform(center_transform.xform(joint.current_tail));
            ...
            if (!chain.joints.empty()) {
                Vector3 end_gizmo = skel_to_gizmo.xform(center_transform.xform(chain.joints.back().current_tail));
