...
Vector3 compute_wind_force(const WindParams &params,
                            const Vector3 &joint_position,
                            float time, int joint_seed) {
    if (params.strength <= 0.0001f || params.direction.is_zero_approx()) {
        return Vector3();
    }
    Vector3 wind_dir = params.direction.normalized();
    
    // Wave phase based on time, joint position projected along wind direction, and seed
    ...
    float perp_wave = Math::sin(perp_phase) * params.turbulence * 0.2f * params.strength;
