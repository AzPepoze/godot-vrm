...
Vector3 closest_point_on_capsule(const Vector3 &head, const Vector3 &tail, const Vector3 &point);
...
Vector3 resolve_capsule_collision(const Vector3 &tail, const Vector3 &origin,
                                   const Vector3 &capsule_head, const Vector3 &capsule_tail,
                                   float joint_radius, float collider_radius,
                                   float joint_length);

// Broad-phase check for sphere collider
inline bool broad_phase_check(const Vector3 &joint_origin, float joint_length, float joint_radius,
                               const Vector3 &collider_pos, float collider_radius) {
...
inline bool broad_phase_capsule_check(const Vector3 &joint_origin, float joint_length, float joint_radius,
                                       const Vector3 &capsule_head, const Vector3 &capsule_tail, float collider_radius) {
    Vector3 closest_cap_point = closest_point_on_capsule(capsule_head, capsule_tail, joint_origin);
    float max_dist = joint_length + joint_radius + collider_radius;
    return (joint_origin - closest_cap_point).length_squared() <= max_dist * max_dist;
}
...
Vector3 resolve_all_colliders(
    const Vector3 &tail, const Vector3 &origin,
