...
Vector3 resolve_capsule_collision(const Vector3 &tail, const Vector3 &origin,
    ...
    return resolve_sphere_collision(tail, origin, coll_pos, joint_radius, collider_radius, joint_length);
}
...
Vector3 resolve_all_colliders(
    ...
    for (const auto &cv : colliders) {
        if (cv.is_capsule) {
            if (!broad_phase_capsule_check(origin, joint_length, joint_radius, cv.position, cv.tail_position, cv.radius)) {
                continue;
            }
            result = resolve_capsule_collision(result, origin, cv.position, cv.tail_position, joint_radius, cv.radius, joint_length);
        } else {
            if (!broad_phase_check(origin, joint_length, joint_radius, cv.position, cv.radius)) {
                continue;
            }
            result = resolve_sphere_collision(result, origin, cv.position, joint_radius, cv.radius, joint_length);
        }
    }
    return result;
}
