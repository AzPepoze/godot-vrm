...
func _import_post(gltf_state: GLTFState, root: Node) -> Error:
	...
	var applier: bone_node_constraint_applier = gltf_state.get_additional_data(
		&"BoneNodeConstraintApplier"
	)
	...
	for i in range(len(nodes)):
		var err: Error = my_import_node(
			gltf_state, nodes[i], json_nodes[i], gltf_state.get_scene_node(i)
		)
		if err != OK:
			return err
	return OK
...
func my_import_node(
	gltf_state: GLTFState, gltf_node: GLTFNode, json: Dictionary, node: Node
) -> Error:
	...
	constraint.resource_name = (
		str(gltf_node.resource_name)
		+ " from "
		+ str(gltf_nodes[constraint.source_node_index].resource_name)
	)
	...
	if gltf_nodes[constraint.source_node_index].skeleton != -1:
		var godot_skel: Skeleton3D = (
			gltf_skeletons[gltf_nodes[constraint.source_node_index].skeleton].get_godot_skeleton()
		)
		...
		if constraint.source_bone_name != "":
			for gltf_skel in skeletons:
				if gltf_skel.get_godot_skeleton() == applier_skel:
					constraint.source_node_index = gltf_skel.godot_bone_node[applier_skel.find_bone(
						constraint.source_bone_name
					)]
		else:
			constraint.source_node_index = node_to_index[applier.get_node(
				constraint.source_node_path
			)]
		var target_node_index: int = -1
		if constraint.target_bone_name != "":
			for gltf_skel in skeletons:
				if gltf_skel.get_godot_skeleton() == applier_skel:
					target_node_index = gltf_skel.godot_bone_node[applier_skel.find_bone(
