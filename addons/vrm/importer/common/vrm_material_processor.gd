...
static func process_vrm_material_v0(
    ...
    if blend_mode == 0:  # Opaque
        ...
        if cull_mode == 0:
            godot_shader_name += "_cull_off"  # FIX: was "_cutout_cull_off"
    elif blend_mode == 2:  # Transparent
        godot_shader_name += "_trans"
        if cull_mode == 0:
            godot_shader_name += "_cull_off"  # FIX: was "_trans_cull_off"
    elif blend_mode == 3:  # TransparentWithZWrite
        godot_shader_name += "_trans_zwrite"
        if cull_mode == 0:
            godot_shader_name += "_cull_off"  # FIX: was "_trans_zwrite_cull_off"
